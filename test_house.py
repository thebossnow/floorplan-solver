from layout import (Room, Adj, Footprint, solve, shared_walls, circulation_ok, to_svg,
                     add_closets, place_openings)
import time

fp = Footprint(width=80, height=60)   # 1200 sf (grid-units: 1 unit = 6in)

rooms = [
    Room("Entry",     192, min_dim=12, max_aspect=2.5, edges=["S"]),
    Room("Living",    984, min_dim=24, max_aspect=1.8),
    Room("Kitchen",   640, min_dim=20, max_aspect=2.0),
    Room("Dining",    480, min_dim=20, max_aspect=1.8),
    Room("Hall",      240, min_dim=8,  max_aspect=8.0, needs_exterior=False),
    Room("Primary",   800, min_dim=24, max_aspect=1.6),
    Room("PrimBath",  288, min_dim=12, max_aspect=2.5),
    Room("Bed2",      560, min_dim=20, max_aspect=1.6),
    Room("Bath2",     200, min_dim=10, max_aspect=2.5),
    Room("Utility",   240, min_dim=12, max_aspect=2.5, needs_exterior=False),
]

adj = [
    Adj("Entry", "Living"),
    Adj("Living", "Dining"),
    Adj("Dining", "Kitchen"),
    Adj("Living", "Hall"),
    Adj("Hall", "Primary"),
    Adj("Hall", "Bed2"),
    Adj("Hall", "Bath2"),
    Adj("Primary", "PrimBath"),
    Adj("Kitchen", "Utility"),
]

# every bedroom gets its own closet -- a small interior room forced (Adj is a
# hard constraint) to share a door-sized wall with its bedroom
rooms, adj = add_closets(rooms, adj, ["Primary", "Bed2"], area=88, min_dim=6, max_aspect=3.0)


t = time.time()
plan, status, objective_value, best_objective_bound, wall_time = solve(fp, rooms, adj, time_limit=60)
print(f"status={status}  {time.time()-t:.1f}s  "
      f"objective={objective_value}  bound={best_objective_bound}  solve_wall_time={wall_time:.1f}s")

if plan:
    tot = 0
    for n, r in sorted(plan.items()):
        dims = "+".join(f"{p['x2']-p['x1']}x{p['y2']-p['y1']}" for p in r["parts"])
        print(f"  {n:9s} {dims:9s} = {r['area']:4d} gu^2   (target {r['target']}, "
              f"delta {r['area']-r['target']:+d})")
        tot += r["area"]
    print(f"  total {tot} / footprint {fp.area()}")

    # only leaf rooms are private -- bedrooms must stay non-private so BFS
    # can relay through them to reach their own ensuite bath/closet
    ok, unreachable = circulation_ok(plan, "Entry",
                                     private=("PrimBath", "Bath2",
                                              "PrimaryCloset", "Bed2Closet"))
    print(f"\ncirculation ok: {ok}  unreachable: {unreachable}")
    print(f"shared walls: {len(shared_walls(plan))}")
    openings = place_openings(plan, fp, adj, rooms)
    print(f"openings placed: {len(openings)}")
    print(to_svg(plan, fp, path="plan.svg", openings=openings))
